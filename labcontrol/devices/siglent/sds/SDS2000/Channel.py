...
class SDS2kChannel(Channel):
    ...
    def position(self):
        """Gets the (vertical) position of this channel with respect to the center graticule. 
        Unit of position is divisions (divs).
        """
    ...
    def capture(self)->WaveForm:
        """"Method for transferring the waveform data from scope to compunter. According to the prog manual the recipe for doing 
        something useful with the tranferred data is:
        1. :WAVeform:SOURce C2, dit is vervelend. Een kanaal wordt door SDS2k aangegeven met CHANNEL, maar binnen het commando 
        WAVEFORM wordt een bronkanaal aangegeven met C1 of C2. Dat is inconsequent. dan :WAVeform:DATA?
        2. Send the query to get the parameters of waveform :WAVeform:PRE?
        3. Calculate the voltage value corresponding to the data point.
        Using the formula: voltage value (V) = code value '*' (vdiv /code_per_div) voffset.
        4. Calculate the time value of the data point.
...
class SDS2kWaveForm(WaveForm):
    ...
    def getWVFData(self):
        ...
        self.rawYdata = self.visaInstr.query_binary_values(SCPI["WAVEFORM"]["data?"](), datatype='B', is_big_endian=False, container=np.ndarray)
