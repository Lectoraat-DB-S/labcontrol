...
def createBeginEndRange(theModelStr:str)-> tuple[list, list]:
    ...
    if tmp1.isalpha():
        if modelStr[-3:].isalpha():#
            #de laatste drie tekens zijn beide letters
            mymodelNr = modelStr[3:-3]
            modelLengte = len(mymodelNr) #bijv 2000 heeft
            startModelRange:list =list()
            eindModelRange:list = list()
            startModelRange.append(mymodelNr[0])
            eindModelRange.append(mymodelNr[0])
            for x in range(1, modelLengte):
                startModelRange.append('0')
                eindModelRange.append('9')
            return (startModelRange, eindModelRange)
        elif modelStr[-2:].isalpha():#
