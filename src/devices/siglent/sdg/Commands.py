...
class SDGCommand(object):
    FREQUENCY = WvPARAM.FRQ.name
    AMPLITUDE = WvPARAM.AMP.name
    OFFSET = WvPARAM.OFST.name
    PERIOD = WvPARAM.PERI.name
    PULSEWDITH = WvPARAM.WIDTH.name
    FALLTIME = WvPARAM.FALL.name
    RISETIME = WvPARAM.RISE.name
    ...
    def setWaveParam(channel, param:WaveformParam)->list:
        ...
        if (param.WVTP.name != WVTP.DC.name) and (param.WVTP.name != WVTP.NOISE.name): 
            ...
            returnstr += f"Waveform period: {param.peri}"
            returnstr += f"Waveform offset: {param.offset}"
            returnstr += f"Waveform high level: {param.HLEV}"
            returnstr += f"Waveform low level: {param.LLEV}"
        if (param.WVTP.name == WVTP.RAMP.name):
            returnstr += f"Waveform symmetry: {param.sym}"
        if (param.WVTP.name == WVTP.SQUARE.name) or (param.WVTP.name == WVTP.PULSE.name): 
            returnstr += f"Waveform duty: {param.duty}"
        if (param.WVTP.name != WVTP.DC.name) and (param.WVTP.name != WVTP.NOISE.name) and (param.WVTP.name != WVTP.PULSE.name): 
            returnstr += f"Waveform phase: {param.phase}"
        if (param.WVTP.name == WVTP.NOISE.name):
            returnstr += f"Waveform MEAN: {param.mean}"     
            returnstr += f"Waveform stdev: {param.stdev}"
            returnstr += f"Waveform bandstatus: {param.bstate.name}"
            returnstr += f"Waveform Noise bandwith: {param.bwidth}"
        if (param.WVTP.name == WVTP.PULSE.name):
            returnstr += f"Waveform WIDTH: {param.pulWidth}"
            returnstr += f"Waveform rise time: {param.rise}"
            returnstr += f"Waveform fall time: {param.fall}"
            returnstr += f"Waveform delay: {param.DLY}"
        if (param.WVTP.name == WVTP.PRBS.name):
            returnstr += f"Waveform PBRS length: {param.length}"
            returnstr += f"Waveform PBRS rise/fall time: {param.edge}"
            returnstr += f"Waveform PBRS differential state: {param.diffs.name}"
        return returnstr
