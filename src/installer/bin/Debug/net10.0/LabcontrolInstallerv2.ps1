...
Write-Host "Probeer nu te detecteren of git al op uw computer staat."
#controle of git al op deze computer staat:
$installGit = !(progExists "git")

Write-Host "Probeer nu te dectecteren of 7-zip al op uw computer staat."
...
if (!$OffLineInstall) {
	...
	if ($forceDownload -or ($installGit -and !$GitInstallAvPathExists)) {
		Write-Host "Probeer nu git-scm te downloaden naar c:\temp."
		$gitDownloaded = downloadProg -ProgName "git-scm" -SourceUrl $git_scm_url -dest $dest
		Write-Host "Git-scm is gedownload. Installatie wordt straks uitgevoerd."
	}

	#downloaden van 7zip
	if ($forceDownload -or !$7ZipInstallAvPathExists) {
		Write-Host "Probeer nu 7-zip te downloaden naar c:\temp."
		$7ZipDownloaded = downloadProg -ProgName "7-zip" -SourceUrl $sevenZip_dwl_url -dest $dest
		Write-Host "7-ZIP is gedownload. Installatie wordt straks uitgevoerd."
	}

	#downloaden van winpyton
	if ($forceDownload -or !$WinPyInstallAvPathExists) {
		Write-Host "Probeer nu WinPython installer te downloaden naar c:\temp."
		$WinPythonDownloaded = downloadProg -ProgName "WinPython" -SourceUrl $WinPython_url -dest $dest
		Write-Host "WinPyton installer is gedownload. Installer wordt straks uitgevoerd."
	}
}
if ($NoInstall) {
	Write-Host "Only Download selected. Quit now...."
	return -1
}
...
if ($installGit -and ($gitDownloaded -or $GitInstallAvPathExists)) {
	Write-Host 'Installatie Git wordt  gestart. A.u.b. toestemming verlenen als daar om gevraagd wordt.\n';
...
if ($install7Zip -and ($7ZipDownloaded -OR $7ZipInstallAvPathExists)) {
	Write-Host 'Installatie 7zip wordt gestart. Kies voor de default instelling en verleen toestemming als daar om gevraagd wordt.\n';
	cmd /c c:\temp\7z2501-x64.exe /S
}

# uitpakken van winpython
if ($WinPythonDownloaded -OR $WinPyInstallAvPathExists) {
	Write-Host 'Installatie WinPython wordt gestart. Kies voor de default instelling en verleen toestemming indien daar om gevraagd wordt.\n';
	...
	Write-Output "uitpakken WinPython gedaan"
